import React from 'react'
import AppLayout from "../../../../components/Layout/Layout";
import Head from "next/head";
import Breadcrumbs from "../../../../components/Layout/Breadcrumbs";
import {useRouter} from "next/router";
import {useData} from "../../../../utils/hooks/useData";
import ContainerTable from "../../../../components/Tables/ContainerTable";

export default function UnprocessedMaterialsContainerView() {
  const router = useRouter();
  const {series} = router.query;
  const {data} = useData(series ? `/v1/archival_unit/${series}/` : null);

  const breadcrumbData = [
    {text: 'Finding Aids'},
    {text: 'Unprocessed Materials', link: '/finding-aids/unprocessed-materials'},
    {text: data ? `${data.title_full}` : ''},
    {text: 'Containers'}
  ];

  return (
    <AppLayout>
      <Head>
        <title>AMS - Archival Management System - Unprocessed Materials / Containers</title>
      </Head>
      <Breadcrumbs module={'finding-aids/containers'} breadcrumbData={breadcrumbData}/>
      <ContainerTable
        seriesID={series}
        seriesTitle={data ? `${data.title_full}` : ''}
        unprocessedMaterials
      />
    </AppLayout>
  )
}
